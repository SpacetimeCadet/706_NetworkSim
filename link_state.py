...
from testcode_linkst import * #remove later
import dictionary as d

def Dijsktra(graph1, src, dest):
    """
    (dict, str, str) -> list
    ...
    """
    graph = d.node_string(graph1)

    sourceV = str(src)
    destinationV = str(dest)
    ...
    node_path = node_states[destinationV].getPred() + list(destinationV)
    node_list = [eval(i) for i in node_path]
    #print(node_list)
    return(node_list)
